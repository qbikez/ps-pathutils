...
Describe "relative path feature" {
    ...
    It "`nPath1:    <path1>`nPath2:    <path2>`nExpected: <expected>`nExisting: <create>" -TestCases $cases {
    ...
    Context "absolute paths" {
        It "`nPath1:    <path1>`nPath2:    <path2>`nExpected: <expected>" -TestCases $cases {
    ...
    Context "absolute paths with alt separator" {
        It "`nPath1:    <path1>`nPath2:    <path2>`nExpected: <expected>" -TestCases $cases {
            param($path1, $path2, $expected)
            # make sure these paths exists, because only then will they will be resolved to full paths
            TouchFile $path1, $path2
...
Describe "where is" {
    ...
    It "Should return array for multiple matches" {
        ...
        (get-item "testdrive:\bin1").fullname | add-topath
        (get-item "testdrive:\bin2").fullname | add-topath
        $w = @(where-is "test321")
